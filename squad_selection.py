...
def get_combinations(collection, gameweek_no, budget):
    points_limits = get_points_limits(collection, gameweek_no)
    ...
    while not combinations:
        ...
        fetched_players = fetch_players(collection, gameweek_no, points_limits)
    ...
    for key, value in myconstants.ELEMENT_TYPES.items():
        top_player = list(
            collection.find({"$and": [{"element_type_id": value["TYPE_ID"]}, {"fixture_history": {"$exists": True}}]},
                ...
                [("fixture_history.%i.%s" % (gameweek_no, "forecast"), -1)]).limit(1))
        points_limits[key] = top_player[0]["fixture_history"][str(gameweek_no)]["forecast"]

    return points_limits


def fetch_players(collection, gameweek_no, points_limits):
    all_players = {}
    ...
    for key, value in myconstants.ELEMENT_TYPES.items():
        ...
        while not enough_players:
            all_players[key] = list(collection.find({"$and": [{"element_type_id": value['TYPE_ID']}, {
                "fixture_history.%i.opposition" % (gameweek_no): {'$ne': '-'}}, {"fixture_history.%i.%s" % (
            gameweek_no, "forecast"): {'$gte': point_limit}}, {"fixture_history.%i.%s" % (
            gameweek_no, "forecast"): {'$exists': True}}, {"fixture_history.%i.net_transfers" % gameweek_no: {
                "$gte": 0}}]}, {"_id": 0, "id": 1, "fixture_history.%i.%s" % (gameweek_no, "forecast"): 1,
                                "fixture_history.%i.value" % gameweek_no: 1, "web_name": 1, "team_name": 1,
                                "element_type_id": 1}))
    ...
    for key, value in all_players.items():
        ...
        for p in value:
            holder.append([p['id'], p['element_type_id'], p['web_name'], p['team_name'],
                           p['fixture_history'][str(gameweek_no)]['value'],
                           p['fixture_history'][str(gameweek_no)]['forecast']])
