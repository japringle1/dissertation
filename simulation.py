...
def run_simulation():
    ...
    for gw in range(startGW, myconstants.GW_COUNT + 1):
        ...
        if (gw == startGW):
            ...
            squad = list(
                myconstants.SQUADS_COLLECTION.find({"$and": [{"gw": gw - 1}]},
                                                   {"_id": 0, "gw": 1, "next_week_cost": 1}))
        ...
        players = myconstants.PLAYER_COLLECTION.find(
            {"$and": [{"info.id": {'$in': ids[-1]}}, {"fixture_history.%i.total_points" % gw: {'$exists': True}}]},
        ...
        for p in players:
            ...
            if ("fixtures" in p["fixture_history"][str(gw)]):
                points += p["fixture_history"][str(gw)]["total_points"]
            points += p["fixture_history"][str(gw)]["total_points"]

            actual_points += int(points)
            actuals[p["info"]["id"]] = {"actual_points": int(points),
                                "mins_played": p["fixture_history"][str(gw)]["minutes"]
                                }
